...
require.define("/node_modules/d3/package.json",function(require,module,exports,__dirname,__filename,process){module.exports = {"main":"index.js","browserify":"index-browserify.js"}});

require.define("/node_modules/d3/index-browserify.js",function(require,module,exports,__dirname,__filename,process){require("./d3.v2");
module.exports = d3;});

require.define("/node_modules/d3/d3.v2.js",function(require,module,exports,__dirname,__filename,process){(function() {
...
require.define("/heapq.coffee",function(require,module,exports,__dirname,__filename,process){(function() {
  var heapq;

  heapq = {
    version: "0.0.0",
    getter: function(heap, index) {
      return heap[index];
    },
    setter: function(heap, index, value) {
      return heap[index] = value;
    },
    compareLessThan: function(x, y) {
      return x < y;
    },
    ...
    _siftDown: function(heap, startPos, pos) {
    ...
    _siftUp: function(heap, pos) {
      ...
      while (childPos < endPos) {
        rightPos = childPos + 1;
        if (rightPos < endPos && !heapq.compareLessThan(heapq.getter(heap, childPos), heapq.getter(heap, rightPos))) {
          childPos = rightPos;
        }
...
require.define("/event-heap.coffee",function(require,module,exports,__dirname,__filename,process){(function() {
  var Backbone, Heapq, heap, _;
  ...
  Heapq.setter = function(heap, index, value) {
    heap.trigger('set', index, value);
    value.index = index;
    return heap[index] = value;
  };

  Heapq.getter = function(heap, index) {
...
require.define("/heap-vis.coffee",function(require,module,exports,__dirname,__filename,process){(function() {
  var $g, BOX_SIZE, HEIGHT, Heapq, STEP, addItem, d3, data, drawHeap, drawLink, heap, rectAttrs, scaleRGB, tree, values, _, _ref;
  ...
  _ref = (function() {
    var g, height, margin, outerHeight, svg, width;
    margin = {
      left: 10,
      right: 440,
      top: 10,
    ...
    svg = d3.select('body').append('svg').attr('class', 'heap-svg').attr("width", width + margin.left + margin.right).attr("height", height + margin.top + margin.bottom).style("margin", "1em 0 1em " + -margin.left + "px");
    g = svg.append('g').attr("transform", "translate(" + margin.left + "," + margin.top + ")");
    return [g, height];
  })(), $g = _ref[0], HEIGHT = _ref[1];

  BOX_SIZE = 20;

  STEP = 120;

  tree = d3.layout.tree().children(function(datum, depth) {
    var children;
  ...
  drawLink = function($link) {
    return $link.attr('d', d3.svg.diagonal().source(function(datum) {
      return {
        x: datum.source.x,
    ...
    }).projection(function(datum) {
      return [datum.y, datum.x];
    }));
  };

  rectAttrs = function($rect, options) {
    return $rect.attr('rx', 6).attr('ry', 6).attr("x", 0).attr("y", -10).attr("height", options.boxSize).attr("width", options.boxSize).style("fill", function(datum) {
      return scaleRGB(datum.value);
    });
  };

  drawHeap = function() {
    var $links, $nodes, indentTree, links, nodes, root;
    root = heap[0];
    nodes = tree.nodes(root);
    links = tree.links(tree.nodes(root));
    $links = $g.selectAll('.link').data(links, function(d) {
      return d.value;
    });
    $links.enter().insert('path', '.node').attr('class', 'link').call(drawLink);
    $links.call(drawLink);
    $links.exit().remove();
    $nodes = $g.selectAll('.node').data(nodes, function(d) {
      return d.value;
    });
    indentTree = function(datum) {
      return "translate(" + (datum.depth * STEP) + "," + datum.x + ")";
    };
    $nodes.enter().append('g').attr('class', 'node').attr('transform', indentTree).append('rect').call(rectAttrs, {
      boxSize: BOX_SIZE
    });
  ...
  scaleRGB = d3.scale.ordinal().domain(values).range(colorbrewer.Blues[values.length]);

  data = _.map(values, function(value, index) {
    return {
      index: index,
      value: value,
      valueOf: function() {
  ...
  addItem = function(datum) {
    datum.index = heap.length;
    Heapq.heapPush(heap, _.extend({}, datum));
    _.each(heap, function(item) {
      return delete item.children;
    });
    return drawHeap(heap);
  };

  (function() {
    var leftPadding, separation, svg;
    svg = d3.select('body').insert('svg', '.heap-svg').attr("width", '150px').attr("height", '3em').style("margin", "0 0");
    leftPadding = 10;
    separation = 50;
    return svg.selectAll('.insert').data(data, function(d) {
      return d.value;
    }).enter().append('rect').attr('class', 'data-add').call(rectAttrs, {
      boxSize: BOX_SIZE
    }).attr("x", function(datum, index) {
      return leftPadding + index * separation;
    }).attr("y", 5.5).on('click', addItem);
  })();

}).call(this);
});
require("/heap-vis.coffee");
})();
